...
record_script_source="${project_dir}/talk2paste-record"
dispatch_script_source="${project_dir}/talk2paste-stop"
record_script_target="${bin_dir}/talk2paste-record"
dispatch_script_target="${bin_dir}/talk2paste-stop"
...
if [ "$action" = 'uninstall' ]; then
  ...
  rm -f "$record_script_target"
  rm -f "$dispatch_script_target"
...
Removed runtime scripts: ${record_script_target}, ${dispatch_script_target}
...
if [ "${#missing_packages[@]}" -gt 0 ]; then
  ...
  printf '  %s\n' "${missing_packages[@]}" >&2
  printf '\ninstall them manually, for example:\n' >&2
  printf '  sudo apt-get update && sudo apt-get install' >&2
...
if [ ! -f "$record_script_source" ] || [ ! -f "$dispatch_script_source" ]; then
  printf 'missing runtime script in project directory: %s or %s\n' "$record_script_source" "$dispatch_script_source" >&2
  exit 1
fi
...
if [ -f "$service_path" ]; then
  ...
  systemctl --user cat "$service_name" >/dev/null 2>&1; then
  service_exists='true'
fi
...
install -m 0755 "$record_script_source" "$record_script_target"
install -m 0755 "$dispatch_script_source" "$dispatch_script_target"
...
Example Sway config:
  bindsym --release F8 exec ${bin_dir}/talk2paste-stop
  bindsym F8 exec ${bin_dir}/talk2paste-record
EOF
